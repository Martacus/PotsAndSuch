...
public class DocheioItems {
    ...
    public static void createPots() {
        Arrays.stream(PotColor.values()).toList().forEach(p -> {
            ...
            POT_JUG_LARGE_MAP.put(p, ITEMS.register("pot_jug_large_" + p.getSerializedName(), () -> new MultiBlockItem(BlockRegistry.POT_JUG_LARGE_MAP.get(p).get(), getPotProperty(), PotJugLargeEntity.getItemStructure(p))));
